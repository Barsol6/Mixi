...
public class PdfController : ControllerBase
{
    ...
    {
        if (FormFile.Length > 20 * 1024 * 1024) return BadRequest("File size exceeds limit");

        if (FormFile == null || FormFile.Length == 0) return BadRequest("No file uploaded");

        if (FormFile.ContentType != "application/pdf") return BadRequest("File is not a PDF");
    ...
    public async Task<IActionResult> GetPdf(string id)
    {
        ...
        if (pdfContent == null) return NotFound();
    ...
    public async Task<IActionResult> UpdatePdfFormData(string id, [FromBody] FormDataUpdateDto dto)
    {
        ...
        if (dto == null || string.IsNullOrWhiteSpace(dto.Data)) return BadRequest("Form data is missing or empty");
    ...
    public async Task<ActionResult<IEnumerable<PdfListItemDto>>> GetPdfList()
    {
        ...
        if (documents == null) return Ok(new List<PdfListItemDto>());
    ...
    public async Task<IActionResult> DeletePdf(string id)
    {
        ...
        if (!success) return NotFound($"Document with ID {id} not found");
