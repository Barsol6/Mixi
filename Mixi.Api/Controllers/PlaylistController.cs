...
public class PlaylistController : ControllerBase
{
    ...
    public PlaylistController(IPlaylistRepository playlistRepository, MetadataService meta)
    {
        ...
        [FromBody] CreatePlaylistItemDto playlistItemDto)
    {
        ...
        if (newPlaylistItemDto == null) return NotFound("Playlist not found");
    ...
    public async Task<ActionResult<CreatePlaylistItemDto>> GetMetadata([FromBody] string url)
    {
        var metadata = await _meta.FetchMetadataAsync(url);
        return Ok(metadata);
    }
}